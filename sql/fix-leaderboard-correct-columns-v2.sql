...
-- Check yesterday too (with proper date casting)
SELECT COUNT(*) as total_completions_yesterday
FROM get_public_daily_leaderboard((CURRENT_DATE - INTERVAL '1 day')::DATE);

-- Alternative way to check yesterday
SELECT COUNT(*) as total_completions_yesterday
FROM get_public_daily_leaderboard(CURRENT_DATE - 1);

-- See data for the last few days directly from the table
SELECT
    puzzle_date,
    COUNT(*) as completions
FROM daily_completions
WHERE puzzle_date >= CURRENT_DATE - 7
GROUP BY puzzle_date
ORDER BY puzzle_date DESC;

-- Check a specific recent date (October 23, 2025)
SELECT COUNT(*) as oct_23_completions
