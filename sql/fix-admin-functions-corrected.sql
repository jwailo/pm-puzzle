-- CORRECTED comprehensive fix for admin dashboard
-- First run the check structure query to see what columns exist, then use this
...
-- 3. Fix the daily completions function with correct column names
...
BEGIN
    RETURN QUERY
    SELECT
        gs.date::DATE as completion_date,  -- Using 'date' column, not 'puzzle_date'
        gs.user_id,
        ...
        gs.completion_time
    FROM game_sessions gs
    LEFT JOIN user_profiles up ON gs.user_id = up.id  -- Fixed join: was up.user_id, should be up.id
...
-- 4. Also create a fallback that tries both column names
CREATE OR REPLACE FUNCTION get_daily_puzzle_completions_safe()
RETURNS TABLE (
    completion_date DATE,
    user_id UUID,
    email TEXT,
...
AS $$
DECLARE
    has_puzzle_date boolean;
    has_date boolean;
BEGIN
    -- Check which columns exist
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'game_sessions' AND column_name = 'puzzle_date'
    ) INTO has_puzzle_date;

    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'game_sessions' AND column_name = 'date'
    ) INTO has_date;

    IF has_puzzle_date THEN
    ...
    ELSIF has_date THEN
        ...
        ORDER BY gs.date DESC, gs.updated_at ASC';
    ELSE
        -- If neither column exists, use created_at as fallback
        RETURN QUERY
        SELECT
            gs.created_at::DATE as completion_date,
            gs.user_id,
            COALESCE(up.email, 'Guest Player') as email,
            COALESCE(up.first_name, 'Guest') as first_name,
            gs.updated_at as completed_at,
        ...
        WHERE gs.game_won = true
            AND gs.user_id IS NOT NULL
        ORDER BY gs.created_at DESC, gs.updated_at ASC;
    END IF;
END;
$$;

-- 5. Update the total users count to be accurate
...
GRANT EXECUTE ON FUNCTION get_daily_puzzle_completions_safe() TO anon, authenticated;
GRANT EXECUTE ON FUNCTION get_admin_total_users() TO anon, authenticated;

-- 7. Verify the fix by checking for the specific user
SELECT * FROM get_admin_user_list()
WHERE email LIKE '%hollierobertson%';

-- 8. Check overall stats
SELECT
